...
def sync_accounts_by_filter(bookmark_prop, fil):
    ...
    accounts = request (gen_request(get_url(endpoint, query='view/'+str(fil_id)+'?include=owner&sort='+bookmark_prop+'&sort_type=desc')))
    for acc in accounts:
...
def sync_contacts_by_filter(bookmark_prop, fil):
    ...
    state_entity = endpoint + "_" + str(fil_id)
    start = get_start(state_entity)
    contacts = request (gen_request(get_url(endpoint, query='view/'+str(fil_id)+'?include=owner&sort='+bookmark_prop+'&sort_type=desc')))
    for con in contacts:
...
def sync_deals_by_filter(bookmark_prop, fil):
    ...
    state_entity = endpoint + "_" + str(fil_id)
    start = get_start(state_entity)
    deals = request(gen_request(get_url(endpoint, query='view/'+str(fil_id)+'?include=owner&sort='+bookmark_prop+'&sort_type=desc')))
    for deal in deals:
...
def sync_leads_by_filter(bookmark_prop, fil):
    ...
    state_entity = endpoint + "_" + str(fil_id)
    start = get_start(state_entity)
    leads = request (gen_request(get_url(endpoint, query='view/'+str(fil_id)+'?include=owner&sort='+bookmark_prop+'&sort_type=desc')))
    for lead in leads:
