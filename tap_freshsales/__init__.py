...
def sync_accounts():
    ...
    singer.write_schema(
        endpoint, schema, ["id"], bookmark_properties=[bookmark_property]
    )
...
def sync_sales_activities():
    ...
    for sale in sales:
        if sale[bookmark_property] >= start:
            ...
            singer.write_record(
                "sale_activities", sale, time_extracted=singer.utils.now()
            )
...
def sync_appointments_by_filter(bookmark_property, fil):
    ...
    appts = gen_request(
        get_url(
            endpoint, filter=fil, include="creater,targetable,appointment_attendees"
        )
    )
